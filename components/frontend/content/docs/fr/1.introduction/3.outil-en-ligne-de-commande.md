...
## Installation automatique (Linux et MacOS, méthode recommandée)

Pour installer l'outil en ligne de commande, vous pouvez utiliser le script d'installation automatique:

```bash
bash -c "$(curl https://dutyduck.net/api/cli-releases/install.sh)"
```

Le script téléchargera la dernière version disponible pour votre plateforme, l'installera dans le répertoire `~/.dutyduck/bin/` et ajoutera ce répertoire à votre PATH. Nous vous
recommandons de lancer ce script en tant qu'utilisateur normal, et non en tant que root.

## Installation manuelle (Linux, MacOS et Windows)

Cette méthode consiste à télécharger le binaire pour votre système d'exploitation et pour votre architecture, et le placer dans le répertoire de votre choix. Elle est recommandée pour
les utilisateurs de Windows et les utilisateurs avancés.
