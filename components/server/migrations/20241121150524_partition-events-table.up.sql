...
ALTER TABLE IF EXISTS incident_timeline_events
...
CREATE TABLE IF NOT EXISTS incident_timeline_events (
    ...
    event_payload jsonb,
    CONSTRAINT fk_incident_id FOREIGN KEY (organization_id, incident_id) REFERENCES incidents (organization_id, id) ON delete cascade
) PARTITION BY RANGE (created_at);

-- Add an index to retrieve events for a given incident efficiently
CREATE INDEX IF NOT EXISTS incident_timeline_events_incident_idx ON incident_timeline_events(organization_id, incident_id);
