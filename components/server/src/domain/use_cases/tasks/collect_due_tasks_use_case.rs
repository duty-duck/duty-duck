    ...
    TRR: TaskRunRepository<Transaction = TR::Transaction>,
{
    ...
    async fn collect_due_tasks(&self) -> anyhow::Result<usize> {
        ...
        let task_aggregates: Vec<TaskAggregate> = self
            .task_repository
            .list_due_tasks(&mut transaction, now, self.select_limit)
            .await
        ...
        for aggregate in task_aggregates {
            let due_aggregate_result = match aggregate {
            ...
            match due_aggregate_result {
                ...
                Err(e) => {
                    error!(
                        error = ?e,
                        "Failed to mark task aggregate as due. This is likely a bug in the SQL query used to retrieve aggregates"
                    );
                }
            }
        }
