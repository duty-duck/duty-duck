...
- Make sure these user attributes exist in the Realm settings, in the "User profile" tab:
    - phoneNumber
    - phoneNumberVerified
    - phoneNumberOtp
- Check e-mail server configuration
  - For the local dev environment, there is a maildev instance available at mailserver:1025
...
and make sure this client scope is enabled for the `dutyduck-dashboard` client as default mappers
...
Make sure there is a "phone" client scope with:
  ...
  - a "phone number verified" mapper
      - token claim name: phoneNumberVerified (camelCase!)
      - Claim JSON type: Boolean
  - a "phoneNumberOtp" mapper
      - token claim name: phoneNumberOtp
      - Claim JSON type: JSON

#### Activate client scopes

Make sure the "active_organization", the "dutyduck-dashboard" and the "phone" client scopes are enabled for the "dutyduck-dashboard" client
