...
const secondsInterval = [10, 30].map((seconds) => ({
  seconds,
  label: t(`dashboard.intervalInput.seconds`, { count: seconds }),
  shortLabel: t(`dashboard.intervalInput.secondsShort`, { count: seconds }),
}));
const minutesInterval = [1, 5, 10, 30].map((minutes) => ({
  seconds: minutes * 60,
  label: t(`dashboard.intervalInput.minutes`, { count: minutes }),
  shortLabel: t(`dashboard.intervalInput.minutesShort`, { count: minutes }),
}));
const hoursInterval = [1, 2, 6, 12, 24].map((hours) => ({
  seconds: hours * 3600,
  label: t(`dashboard.intervalInput.hours`, { count: hours }),
  shortLabel: t(`dashboard.intervalInput.hoursShort`, { count: hours }),
}));
