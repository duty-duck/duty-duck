  ...
  <BCard header-tag="nav" no-body>
    <template #header>
      <BNav card-header tabs>
        <BNavItem
          @click="emits('changeTab', 'ongoing')"
          :active="currentTab == 'ongoing' && !!onGoingIncident"
          :disabled="!onGoingIncident"
        >
          <Icon
            v-if="onGoingIncident"
            name="ph:seal-warning-duotone"
            class="text-danger"
            size="1.2rem"
        ...
        <BNavItem
          @click="emits('changeTab', 'history')"
          :active="currentTab == 'history' || !onGoingIncident"
        >
    ...
    <BCardBody v-if="currentTab == 'ongoing' && onGoingIncident">
      ...
      <p>
        <span class="text-secondary"
          >{{ $t("dashboard.incidents.rootCause") }}:</span
        ><br />
        <template
          v-if="onGoingIncident.cause?.causeType == 'HttpMonitorIncidentCause'"
        >
          <p>{{ $t("dashboard.httpMonitorIncidents.httpMonitorFailure") }}</p>
          <HttpMonitorIncidentLabel :incident="onGoingIncident" />
        </template>
      </p>
    </BCardBody>
    ...
    <template v-else-if="incidents.data">
      <BListGroup flush>
        <BListGroupItem
          href="#"
          v-for="i in incidents?.data?.items"
          :key="i.id"
        >
      ...
      <BCardBody>
        <BPagination
          :modelValue="incidentsPageNumber"
          @update:modelValue="(page: number) => emits('changePage', page)"
          :prev-text="$t('pagination.prev')"
          :next-text="$t('pagination.next')"
          :total-rows="incidents!.data!.totalNumberOfFilteredResults"
          :per-page="10"
        />
      </BCardBody>
    </template>
